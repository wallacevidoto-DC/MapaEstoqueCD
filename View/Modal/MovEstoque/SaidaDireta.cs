...
namespace MapaEstoqueCD.View.Modal
{
    public partial class SaidaDireta : Form
    {
        ...
        {
            saidaDiretaDto = new SaidaDiretaDto
            {
        ...
        private void button_salvar_Click(object sender, EventArgs e)
        {
            ...
            {
                if (isEntradaConferencia && saidaDiretaDto is not null)
                {
                    saidaDiretaDto.dataEntrada = dateTimePicker_dataEntrada.Value;
                    saidaDiretaDto.observacao = richTextBox_obs.Text;
                    saidaDiretaDto.produtos = produtoSpDtos.Where(p => p.propsPST.origem == Origem.OUT).ToList();
                    if (!string.IsNullOrEmpty(CifsName))
                    {
                        saidaDiretaDto.observacao = $"CIF: {CifsName} - {saidaDiretaDto.observacao}";
                    }
                }
                else
                {
                ...
                if (estoqueController.SetSaidaDireta(saidaDiretaDto))
                {
                    MessageBox.Show(Text = "Saída direta registrada com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    var dr = MessageBox.Show("Deseja registrar outra saída direta?", "Continuar", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
